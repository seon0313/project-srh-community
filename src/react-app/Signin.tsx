...
function Signin() {
    ...
    const getPasswordClass = () => {
        if (!password) return style.input;
        return password.length >= 6 ? `${style.input} ${style.valid}` : `${style.input} ${style.invalid}`;
    };

    const getPasswordReClass = () => {
        if (!passwordre) return style.input;
        return password === passwordre ? `${style.input} ${style.valid}` : `${style.input} ${style.invalid}`;
    };
        ...
        <>
            <div className="Signin">
                ...
                <div className={style.signinForm}>
                    ...
                    />
                    <input
                        className={getPasswordReClass()}
                        type="password"
                        placeholder="비밀번호 확인"
                        value={passwordre}
                        onChange={(e) => setPasswordre(e.target.value)}
                    />
                    <button className={style.signupButton} onClick={handleSignup}>회원가입</button>
                </div>

                <div className={style.additionalInfo}>
                    <div className={style.instagramLink}>
                        <a href="/signup">인스타 DM으로 편리하게 회원가입</a>
                    </div>
